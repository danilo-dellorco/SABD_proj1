...
public class Testing {
    ...
    */
    ...
    public static void query3_pro(SparkSession spark, String dataset) {
        ...
        JavaPairRDD<Long, ValQ3> statistics = occurrences.mapToPair(
                r -> {
        ...
        JavaPairRDD<Long, Tuple2<ValQ3, Optional<ValQ3>>> joined = occurrences.leftOuterJoin(statistics);

        System.out.println("JOINEEEEEEED");
        joined.foreach((VoidFunction<Tuple2<Long, Tuple2<ValQ3, Optional<ValQ3>>>>) r->System.out.println(r.toString()));


        JavaPairRDD<Long, ValQ3> iterations = joined.mapToPair(
                r -> {
                    Double fare_mean = r._2()._2().get().getFare();
                    Double fare_val  = r._2()._1().getFare();
                    Double fare_dev  = Math.pow((fare_val-fare_mean),2);
                    r._2()._2().get().setFare_stddev(fare_dev);

                    return new Tuple2<>(r._1(), r._2()._2().get());
                });
